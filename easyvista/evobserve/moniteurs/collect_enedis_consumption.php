...
function () use (&$liste_option, &$fichier_log) {
    ...
    try {
        ...
        $resultat = $enedis_ws->consumption_load_curve(array(
        ...
        abstract_log::onDebug_standard(date("Y-m-d H", strtotime($liste_option->getOption("date"))) . ":00:00", 1);
        $search_ts = strtotime(date("Y-m-d H", strtotime($liste_option->getOption("date"))) . ":00:00");
        foreach ($resultat->meter_reading->interval_reading as $compteur) {
            abstract_log::onDebug_standard($compteur, 2);
            if (is_array($compteur->date)) {
                $data_ts = strtotime(substr($compteur->date[0], 0, 13) . ":00:00");
            } else {
                $data_ts = strtotime(substr($compteur->date, 0, 13) . ":00:00");
            }
            abstract_log::onDebug_standard($search_ts . "==" . $data_ts, 2);
            if ($search_ts == $data_ts) {
                abstract_log::onDebug_standard("TS validated", 1);
                if (is_array($compteur->value)) {
                    $consumption += $compteur->value[0];
                } else {
        ...
        $consumption=round($consumption/12/1000,2);
