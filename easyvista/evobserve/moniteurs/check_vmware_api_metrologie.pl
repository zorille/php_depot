...
# Fix pour accepter les certificats SSL auto signés
# http://www.techadre.com/content/fix-checkesx3pl-script
$ENV{'PERL_LWP_SSL_VERIFY_HOSTNAME'} = 0;

# cette modification necessite une version de Bytes::Random::Secure patche sur la machine. Voir VSWA-13954 et VSWA-13735
$ENV{'PERL_BYTES_RANDOM_SECURE_STD_FUNC_NON_BLOCKING'} = 1;
...
sub main {
	...
	#dsvoboda: 0014743: [Commande][check_vmware_api] argument timeout n'autorise pas une valeur vide
	#begin hack: remove the =i argument for replacing it by =s so it supports empty value!!!
	$np->opts->{_args} = [grep { $_->{spec} ne 'timeout|t=i' } @{$np->opts->{_args}}];

	$np->add_arg(
    	spec => 'timeout|t=s',
    	help => "-t, --timeout=INTEGER_OR_EMPTY\n   Seconds before plugin times out (default: %s)",
    	default => 30,
	);
	#end hack!!!
	...
	# MODIF VSWA-21104
	$np->add_arg(
		spec => 'exclure_sensor|e=s',
		help => "-e, --exclure_sensor=<exclure_sensor>\n"
		. 'Exclude some sensors that has no longer reassembled by VMWare from version 6.7',
		default => 0,
		required => 0,
	);
	# MODIF VSWA-21104
	...
	# MODIF VSWA-21104
	my $wantToExcludeSensors = false;
	if (defined($np->opts->exclure_sensor) && $np->opts->exclure_sensor eq '1') {
		$wantToExcludeSensors = true;
	}
	# END MODIF VSWA-21104
	
	#dsvoboda: 0014743: [Commande][check_vmware_api] argument timeout n'autorise pas une valeur vide
	#if empty timeout => timeout = 30s
	...
	eval
	{
		...
		{
			$np->nagios_exit(OK, "No Host or Datacenter specified");
		}
		...
		if (defined($sessionfile) and $sessionfile ne '' and -e $sessionfile)
		{
			#Trying to resume existing session from sessionfile
			Opts::set_option("sessionfile", $sessionfile);
      		eval {Vim::load_session(session_file => $sessionfile)};

			if ($@) {
				#session resume failed, logging in at host_address as username
				Opts::set_option("sessionfile", undef);
				Util::connect($host_address, $username, $password);
				Vim::save_session(session_file => $sessionfile);
			}
		}else{
			#sessionfile does not exist
			#logging in at host_address as username
			Util::connect($host_address, $username, $password);
			Vim::save_session(session_file => $sessionfile) if ($sessionfile ne '');
		}
		...
		elsif (defined($host))
		{
			...
			{
				($result, $output) = host_runtime_info($esx, $np, local_uc($subcommand), $blacklist, $addopts,$wantToExcludeSensors);
			}
	...
	if(defined($sessionfile) && $sessionfile ne '' && -e $sessionfile)
	{
...
{
	my ($host, $np, $subcommand, $blacklist, $addopts,$wantToExcludeSensors) = @_;
			...
			if(defined($runtime->healthSystemRuntime))
			{
				...
				if (defined($cpuStatusInfo))
				{
					foreach (@$cpuStatusInfo)
					{
						...
						if( $wantToExcludeSensors && $state == 3 ) {						
                           	next;	
						}
					...
					if ($AlertCount > 0) {
						...
						foreach my $fstate (reverse(sort(keys(%$components)))) {
							...
							foreach my $state_ref ( $components->{$fstate}) {
								foreach my $type ( keys(%$state_ref)) {
									foreach my $item_ref (@{$state_ref->{$type}}) {
										if ($wantToExcludeSensors and (($item_ref->{summary} =~ m/Cannot report/i) or ($item_ref->{label}=~ m/unknown/i ) or ($item_ref->{state} == 3))) {
											next;
										} else {
						...
						if( $OKCount == 0 ) {
							$output = "No sensor detected. Please check";
							$res = UNKNOWN;
						}
					}
				}
			...
			if (exists($storage->storageDeviceInfo->{multipathInfo}))
			{
				...
				foreach my $lun (@{$storage->storageDeviceInfo->multipathInfo->lun})
				{
					foreach my $path (@{$lun->path})
					{
						...
						$res = OK if ($normalizedPathState eq "DEAD");
		...
		elsif ($subcommand eq "STATUS")
		{
			...
			foreach my $dc (@$dc_views) {
				...
				else
				{
					...
					$res = Nagios::Plugin::Functions::max_state($res, OK);
				}
			}
			if ($output) {
				chop($output);
