...
sub collectValue {
	verb("collectValue");
	my ($ua ,$token,$url,$company,$host,$service,$position,$periode) = @_ or die "Missing (User Agent,Token or Url) Parametre please check  ";
	my $result;
	...
	my $size = @{$result};
	verb("getData Service ID Value : Result found ". $size);
	my $service_id=0;
	...
	my $sort = "";
	if( length $periode ) {
	#On collecte les valeurs du jour uniquement
	my $t = DateTime->now(time_zone => "local")->subtract(hours => $periode);
		$datestringstart = $t->strftime("%F %H:%M:%S");
	} else {
		$datestringstart = strftime "%Y-%m-01 00:00:00", localtime;
	}
	...
	#collect group metric from service A from company
	$apiUrl = $url ."/bigdata/groups/$service_id/metrics/$metric_id/measurements?date_start=$datestringstart&date_stop=$datestringstop&page=1&limit=1&sort=$sort";
	$result = getData($ua,$token,$apiUrl);
...
$np->addOption({
        key             => 'periode',
        label           => 'Periode en heure',
        type            => 22,
        description => 'Periode entre les collectes. Ce champ s\'exprime en heure.',
        required        => 0,
        sample          => '24'
    });
...
my $periode		= $np->getValue('periode');
...
my $position = "debut";
my $resultatA = collectValue($ua,$token,$url,$companies,$host,$service,$position,$periode);
...
$position="fin";
my $resultatB = collectValue($ua,$token,$url,$companies,$host,$service,$position,$periode);
