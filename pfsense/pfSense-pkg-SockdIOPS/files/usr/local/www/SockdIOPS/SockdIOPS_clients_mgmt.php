...
if (! is_array ( $config ['installedpackages'] ['sockdiops'] ['passclients'] )) {
	$config ['installedpackages'] ['sockdiops'] ['passclients'] = array ();
}
$pass_clients = &$config ['installedpackages'] ['sockdiops'] ['passclients'];
...
if (isset ( $_POST ['ClientPassList_dup'] ) && isset ( $pass_clients [$_POST ['ClientPassList_id']] )) {
	...
	write_config ( "SockdIOPS pkg: Cloned Pass Client entry from list." );
}
if (isset ( $_POST ['ClientPassList_delete'] ) && isset ( $pass_clients [$_POST ['ClientPassList_id']] )) {
	...
	write_config ( "SockdIOPS pkg: deleted pass client entry from list." );
}
...
if (isset ( $_POST ['PassSave'] ) && isset ( $_POST ['ClientPassList_id'] )) {
	...
	if (valide_ip_cidr_fqdn ( $tmp, 'Pass', 'Client','' ) && valide_port ( $tmp, 'Pass', 'Client','s' )) {
		...
		$savemsg = "SockdIOPS pkg:Pass Client created in list.";
...
// Gestion des BLOCK
if (! is_array ( $config ['installedpackages'] ['sockdiops'] ['blockclients'] )) {
	$config ['installedpackages'] ['sockdiops'] ['blockclients'] = array ();
}
$block_clients = &$config ['installedpackages'] ['sockdiops'] ['blockclients'];
...
if (isset ( $_POST ['ClientBlockList_delete'] ) && isset ( $block_clients [$_POST ['ClientBlockList_id']] )) {
	...
	write_config ( "SockdIOPS pkg: deleted block client entry from list." );
}
// Permet de mettre les variable dans l'objet de modification
...
if (isset ( $_POST ['BlockSave'] ) && isset ( $_POST ['ClientBlockList_id'] )) {
	...
	if (valide_ip_cidr_fqdn ( $tmp, 'Block', 'Client','' ) && valide_port ( $tmp, 'Block', 'Client','s' )) {
		...
		$savemsg = "SockdIOPS pkg:Block Client created in list.";
		write_config ( $savemsg );
	}
}
...
<form action="SockdIOPS_clients_mgmt.php" method="post"
