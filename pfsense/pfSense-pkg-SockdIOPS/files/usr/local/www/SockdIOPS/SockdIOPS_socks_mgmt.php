...
if (! is_array ( $config ['installedpackages'] ['sockdiops'] ['passsocks'] )) {
	$config ['installedpackages'] ['sockdiops'] ['passsocks'] = array ();
}
$pass_socks = &$config ['installedpackages'] ['sockdiops'] ['passsocks'];
...
if (isset ( $_POST ['SocksPassList_dup'] ) && isset ( $pass_socks [$_POST ['SocksPassList_id']] )) {
	...
	$pass_socks[$id]['id']=$id;
	write_config ( "SockdIOPS pkg: Cloned Pass Socks entry from list." );
}
if (isset ( $_POST ['SocksPassList_delete'] ) && isset ( $pass_socks [$_POST ['SocksPassList_id']] )) {
	// Write the new configuration
	unset ( $pass_socks [$_POST ['SocksPassList_id']] );
	write_config ( "SockdIOPS pkg: deleted pass socks entry from list." );
}
...
if (isset ( $_POST ['PassSave'] ) && isset ( $_POST ['SocksPassList_id'] )) {
	...
	if (valide_ip_cidr_fqdn ( $tmp, 'Pass', 'Socks','from' ) && valide_ip_cidr_fqdn ( $tmp, 'Pass', 'Socks','to' ) && valide_port ( $tmp, 'Pass', 'Socks','t' )) {
		...
		$savemsg = "SockdIOPS pkg:Pass Socks created in list.";
...
// Gestion des BLOCK
if (! is_array ( $config ['installedpackages'] ['sockdiops'] ['blocksocks'] )) {
	$config ['installedpackages'] ['sockdiops'] ['blocksocks'] = array ();
}
$block_socks = &$config ['installedpackages'] ['sockdiops'] ['blocksocks'];
...
if (isset ( $_POST ['SocksBlockList_delete'] ) && isset ( $block_socks [$_POST ['SocksBlockList_id']] )) {
	...
	write_config ( "SockdIOPS pkg: deleted block socks entry from list." );
}
// Permet de mettre les variable dans l'objet de modification
...
if (isset ( $_POST ['BlockSave'] ) && isset ( $_POST ['SocksBlockList_id'] )) {
	...
	if (valide_ip_cidr_fqdn ( $tmp, 'Block', 'Socks','from' ) && valide_ip_cidr_fqdn ( $tmp, 'Block', 'Socks','to' ) && valide_port ( $tmp, 'Block', 'Socks','t' )) {
		...
		$savemsg = "SockdIOPS pkg:Block Socks created in list.";
		write_config ( $savemsg );
	}
}
...
<form action="SockdIOPS_socks_mgmt.php" method="post"
