...
function principale(&$liste_option, &$fichier_log) {
	...
	$vmware_webservice = VMware\vmwareWsclient::creer_vmwareWsclient ( $liste_option, VMware\vmwareDatas::creer_vmwareDatas ( $liste_option ) );
	...
	$vim25 = VMware\Vim25::creer_Vim25 ( $liste_option, $vmware_webservice );
	...
	foreach ( $liste_VMs as $VM ) {
		try {
			...
			if (isset ( $donnees_Vm ['config'] ['name'] )) {
				...
				$texte = str_replace ( "><", ">\n<", $xml_vm_info->asXML () );
				$file = Core\fichier::creer_fichier ( $liste_option, $fichier, "oui" );
				$file->ouvrir ( "w" );
				$file->ecrit ( $texte );
				$file->close ();
				Core\abstract_log::onInfo_standard ( $fichier );
			}
		} catch ( Exception $e ) {
