 ...
 */
require_once $liste_option->renvoie_option ( "rep_scripts" ) . "/lib/sitescope_tasks_functions.class.php";
/**
 * Librairies specifiques au programme
 */
require_once $liste_option->renvoie_option ( "rep_scripts" ) . "/lib/sitescope_functions_locales.class.php";

function principale(&$liste_option, &$fichier_log) {
	if ($liste_option->verifie_option_existe ( "type_desactivation" ) === false) {
		$liste_option->set_option ( "type_desactivation", "no_force" );
	}
	...
	foreach ( $liste_tasks_serveur as $task_data ) {
		...
		switch ($liste_option->renvoie_option ( "type_desactivation" )) {
			case "moniteur" :
